...
### Now let's analyze the CO2 emissions per capita (co2_PC). Make a similar plot 
...
### Compute the average LE growth from 1960 - 2019 across the continents. Show
### the results in order of growth. Explain what do you see. 

```{r}
gapminder %>% 
  group_by(region) %>% 
  mutate(growth_le = lifeExpectancy - lag(lifeExpectancy)) %>% 
  filter(!is.na(growth_le), 
         !is.na(region)) %>% 
  summarize(avg_growth_le = mean(growth_le)) %>% 
  knitr::kable()

```

Oceanic countries had a higher average growth in life expectancy. 
Africa, Asia, and Europe did not change much. Americas comes in at a hot second
in average growth of life expectancy. 
